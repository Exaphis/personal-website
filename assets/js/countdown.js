...
function initializeClock(endtime) {
  ...
  function updateClock() {
    ...
    var percentFinished = Math.min(100, 100 * (currentTime - start) / timeDelta);
    
    // Make confetti fall if percentage is whole number
    if (Math.abs((Math.floor(percentFinished * 100) / 100) % 1 - 0) < 0.00001) {
      if (!confetti) {
        poof();
      }
      confetti = true;
    ...
    progressBarLine.children[0].style.width = (Math.floor(percentFinished * 100) / 100) + "%";
    
    var numRepeats = Math.floor(((100 - percentFinished) / percentFinished) * 100) / 100;
    ...
    for (var i = 0; i < nextPercents.length; i++) {
      ...
      nextNPercentText.innerHTML += nextPercents[i].toString() + "% at " + percentTime.toString() + "<br />";
    }
  }
...
var start = new Date("18 Aug 2020 12:00:00 PDT")
var deadline = new Date("21 Nov 2020 10:33:00 EST");
